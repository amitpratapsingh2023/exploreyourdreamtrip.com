<?php
/**
 * Dynamic Tour Details Template
 * ─────────────────────────────
 * Renders any tour from the centralized data/tours.php data source.
 * URL pattern: /tours/{slug} → tours/details.php?slug={slug}
 *
 * @requires .htaccess rewrite rule
 * @uses     data/tours.php  (tour detail data)
 * @uses     includes/config.php (base tour card data, site config)
 */

// ─── Load Config (auto-loads data/tours.php with $TOURS) ──
require_once __DIR__ . '/../includes/config.php';

// ─── Resolve Slug ─────────────────────────────────────────────────────
$slug = isset($_GET['slug']) ? trim($_GET['slug']) : '';

// Validate slug exists in the data source
if (empty($slug) || !isset($TOURS[$slug])) {
...
// ─── Load Tour Data ───────────────────────────────────────────────────
$tour        = $TOURS[$slug];          // Unified tour data (both basic card and detail data)
$tour_detail = $tour;                  // Alias to avoid refactoring details template markup

$page_title = $tour['page_title'];
$page_desc  = $tour['page_desc'];

// ─── Dynamic Breadcrumb ───────────────────────────────────────────────
$breadcrumb_custom = [
    ['title' => 'Tours & Packages', 'url' => BASE_URL . 'tours.php', 'active' => false],
    ['title' => $tour['title'], 'url' => '', 'active' => true],
];

// ─── Canonical URL ────────────────────────────────────────────────────
$canonical_url = BASE_URL . 'tours/' . $slug;
...
<script type="application/ld+json">
{
  ...
  "touristType": <?php echo json_encode($tour_detail['schema_tourist_type']); ?>,
  "offers": {
    "@type": "Offer",
    "price": "<?php echo $tour_detail['schema_price']; ?>",
    "priceCurrency": "INR",
...
<section
    ...
    <div class="absolute inset-0 z-0">
        <img src="<?php echo $tour['image']; ?>" alt="<?php echo htmlspecialchars($tour['title']); ?>"
...
<section class="relative z-20 -mt-10 px-4 max-w-[1440px] mx-auto">
    <div class="glass-nav rounded-[28px] border border-brand/20 p-6 md:p-8 shadow-2xl glow-gold">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-6 sm:gap-y-8 lg:gap-y-0 lg:divide-x lg:divide-white/10">
            <?php foreach ($tour_detail['trust_props'] as $prop): ?>
                <div class="px-2 sm:px-4 lg:px-6 flex items-center space-x-4">
                    <div
                        ...
                        <i class="<?php echo $prop['icon']; ?> text-base"></i>
                    </div>
                    <div>
                        <h4 class="text-white text-xs font-extrabold uppercase tracking-wider font-display"><?php echo htmlspecialchars($prop['title']); ?></h4>
                        <p class="text-slate-400 text-[11px] mt-0.5 font-medium leading-tight"><?php echo htmlspecialchars($prop['desc']); ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
                        ...
                        <div class="space-y-8">
                            <div class="space-y-3">
                                ...
                                <h2
                                    ...
                                    <?php echo $tour_detail['overview_heading']; ?>
                                </h2>
                            </div>
                            <p
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                                    <?php foreach ($tour_detail['highlights'] as $highlight): ?>
                                        <div
                                            ...
                                            <span
                                                class="text-sm text-slate-800 font-semibold tracking-wide"><?php echo htmlspecialchars($highlight); ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3.5">
                                    <?php foreach ($tour_detail['key_facts'] as $key => $value): ?>
                                        <div
                                            ...
                                            <p class="text-sm text-obsidian-950 font-bold pl-1"><?php echo htmlspecialchars($value); ?>
                                            </p>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        <div class="space-y-6">
                            <div class="space-y-3">
                                ...
                                <p class="text-slate-500 text-sm md:text-base leading-relaxed">A carefully curated
                                    timeline of your <?php echo htmlspecialchars($tour['title']); ?> adventure.</p>
                            </div>
                                            ...
                                            <div
                                                ...
                                                <button
                                                    ...
                                                    <div class="flex items-center gap-3 sm:gap-4">
                                                        ...
                                                        <div>
                                                            <span
                                                                class="text-[10px] text-brand font-extrabold uppercase tracking-widest"><?php echo htmlspecialchars($step['time']); ?></span>
                                                            <h3
                                    ...
                                    <ul class="space-y-3.5">
                                        <?php foreach ($tour_detail['inclusions'] as $item): ?>
                                            <li class="flex items-start space-x-3.5">
                                                ...
                                                <span
                                                    class="text-slate-700 text-sm font-medium leading-relaxed"><?php echo htmlspecialchars($item); ?></span>
                                            </li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                                    ...
                                    <ul class="space-y-3.5">
                                        <?php foreach ($tour_detail['exclusions'] as $item): ?>
                                            <li class="flex items-start space-x-3.5">
                                                ...
                                                <span
                                                    class="text-slate-600 text-sm font-medium leading-relaxed"><?php echo htmlspecialchars($item); ?></span>
                                            </li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                                    ...
                                    <div
                                        ...
                                        <div class="p-6 md:p-7 space-y-5 flex-grow flex flex-col">
                                            <div class="space-y-3">
                                                ...
                                                <div>
                                                    <h3
                                                        class="text-base font-extrabold text-obsidian-950 font-display leading-tight">
                                                        <?php echo htmlspecialchars($plan['vehicle']); ?></h3>
                                                    <p
                                                        class="text-xs text-slate-400 mt-1 font-semibold uppercase tracking-wider">
                                                        <?php echo htmlspecialchars($plan['capacity']); ?></p>
                                                </div>
                                            </div>
                                            ...
                                            <ul class="space-y-2.5 text-sm text-slate-600 flex-grow pt-2">
                                                <li class="flex items-center space-x-2"><span
                                                        class="text-brand text-[10px]"><i
                                                            class="fa-solid fa-check"></i></span> <span>Roundtrip luxury transport</span></li>
                                                <li class="flex items-center space-x-2"><span
                                                        class="text-brand text-[10px]"><i
                                                ...
                                                <li class="flex items-center space-x-2"><span
                                                        class="text-brand text-[10px]"><i
                                                            class="fa-solid fa-check"></i></span> <span>24/7 travel support</span></li>
                                            </ul>
                    ...
                    <div class="p-6 md:p-8 space-y-6">
                        <?php foreach ($tour_detail['key_facts'] as $key => $value): ?>
                            <div class="flex items-center justify-between py-3.5 border-b border-slate-100 last:border-0">
                                ...
                                <span class="text-sm text-obsidian-950 font-bold"><?php echo htmlspecialchars($value); ?></span>
                            </div>
                        <?php endforeach; ?>
...
<div id="gallery-lightbox"
    ...
    <div id="lightbox-counter"
        class="absolute top-6 left-6 text-white/50 text-xs font-bold uppercase tracking-wider z-10">1 / <?php echo count($tour_detail['gallery']); ?></div>
    <!-- Prev -->
            ...
            foreach ($TOURS as $key => $related_tour):
                ...
                <div
                    ...
                    <div class="relative overflow-hidden aspect-[16/11] z-0">
                        <img src="<?php echo $related_tour['image']; ?>" alt="<?php echo htmlspecialchars($related_tour['title']); ?>"
                            class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105"
                            loading="lazy">
                        ...
                        <div class="absolute bottom-5 left-6 right-6">
                            <span
                                class="text-brand-light text-[10px] font-extrabold uppercase tracking-widest"><?php echo htmlspecialchars($related_tour['subtitle']); ?></span>
                            <h3 class="text-white text-xl font-bold font-display mt-1 leading-tight tracking-tight">
                                <?php echo htmlspecialchars($related_tour['title']); ?>
                            </h3>
                        </div>
                    </div>
                    <!-- Body -->
