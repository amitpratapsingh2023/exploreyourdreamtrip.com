...
<head>
    ...
    <title>Admin Login — Veda Webtech</title>
...
<body class="min-h-screen bg-dark-900 flex items-center justify-center p-4 relative overflow-hidden font-sans">
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-1/4 -left-20 w-[500px] h-[500px] bg-gold-400/10 rounded-full blur-[150px] animate-float"></div>
        <div class="absolute bottom-1/4 -right-20 w-[600px] h-[600px] bg-gold-600/8 rounded-full blur-[180px] animate-float-delayed"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gold-500/5 rounded-full blur-[200px] animate-pulse-glow"></div>
        <!-- Grid pattern -->
        <div class="absolute inset-0 opacity-[0.03]" style="background-image: radial-gradient(circle, rgba(244,208,63,0.3) 1px, transparent 1px); background-size: 40px 40px;"></div>
    </div>
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="text-center mb-10">
            <div class="inline-flex items-center justify-center mb-6">
                <img src="<?= SITE_URL ?>/assets/images/brand/logo.png" alt="Veda Webtech" class="h-14 w-auto dark:brightness-0 dark:invert opacity-90 hover:opacity-100 transition-opacity">
            </div>
            <h1 class="font-heading text-3xl font-extrabold text-white tracking-tight">Admin Panel</h1>
            <p class="text-gray-500 text-sm mt-2 font-medium">Veda Webtech Management Console</p>
        </div>

        <!-- Card -->
        <div class="bg-dark-800/60 backdrop-blur-2xl border border-white/[0.06] rounded-3xl p-8 md:p-10 shadow-[0_20px_60px_rgba(0,0,0,0.5)]">

            <!-- Error Messages -->
            <?php if ($error): ?>
            <div class="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center gap-3 text-red-400 text-sm font-medium animate-[fadeIn_0.3s_ease]" id="login-error">
            ...
            <div class="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3 text-amber-400 text-sm font-medium">
            ...
            <form method="POST" action="" id="loginForm" class="space-y-6" novalidate>
                ...
                <!-- Email -->
                <div>
                    <label for="email" class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Email Address</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-600 group-focus-within:text-gold-400 transition-colors">
                        ...
                        <input type="email" id="email" name="email" value="<?= e($_POST['email'] ?? '') ?>" required autocomplete="email"
                            ...
                            placeholder="admin@vedawebtech.com">
                    </div>
                ...
                <!-- Password -->
                <div>
                    <label for="password" class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Password</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-gray-600 group-focus-within:text-gold-400 transition-colors">
                ...
                <button type="submit" id="loginBtn"
                    class="w-full bg-gradient-to-r from-gold-400 to-gold-500 text-dark-900 font-extrabold py-4 rounded-2xl
        ...
        <p class="text-center text-gray-700 text-xs mt-8 font-medium">
            &copy; <?= date('Y') ?> Veda Webtech. Secure Admin Access.
        </p>
    </div>
